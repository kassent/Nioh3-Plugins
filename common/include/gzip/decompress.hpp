...
class Decompressor
{
  ...
  public:
    Decompressor(int window_bits = 15 + 32, std::size_t max_bytes = 1000000000) // by default refuse operation if compressed data is > 1GB
        : window_bits_(window_bits), max_(max_bytes)
    {
    }
...
#pragma GCC diagnostic ignored "-Wold-style-cast"
        if (inflateInit2(&inflate_s, window_bits_) != Z_OK)
        {
...
inline std::string decompress(const char* data, std::size_t size, int windowBits = 15 + 32)
{
    Decompressor decomp(windowBits);
    std::string output;
