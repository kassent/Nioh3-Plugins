...
class Compressor
{
  ...
  public:
    Compressor(int level = Z_DEFAULT_COMPRESSION, int window_bits = 15 + 16,
                  ...
                  std::size_t size) const
    {
        ...
        // constexpr int window_bits = windowBits; // gzip with windowbits of 15
...
#pragma GCC diagnostic ignored "-Wold-style-cast"
        if (deflateInit2(&deflate_s, level_, Z_DEFLATED, window_bits_, mem_level, Z_DEFAULT_STRATEGY) != Z_OK)
        {
...
{
    Compressor comp(level, windowBits);
    std::string output;
