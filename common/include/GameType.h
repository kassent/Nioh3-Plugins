...
struct ItemDataManager : DataArrayManager<ItemData> {
    DEF_MEMBER_FN_REL_CONST(GetItemData, ItemData*, 0x04A25F0, "E8 ? ? ? ? 45 33 C0 48 85 C0 74 ? 48 8B 87", 0, 1, 5, uint16_t a_itemId);
};
...
// 0223.exe
// Pointer chain (IDA):
// qword_144566990 -> GameMain
// GameMain + 0x530 -> RdbBootstrapRuntime*
// RdbBootstrapRuntime + 0x210 -> a1 (AssetManager)
struct GameAsset {
    ...
    std::uint32_t flags = 0;           // +0x30 RDB/RIS flags (compression mode in high bits) seems callback ptr
